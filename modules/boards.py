...
from types import MondayApiResponse, Item, BoardKind, BoardState, BoardsOrderBy
...
class BoardModule(MondayGraphQL):
    ...
    def fetch_item_by_board_id_by_update_date(
        ...
        if not updated_after and not updated_before:
            raise ValueError("Either updated_after or updated_before must be provided when fetching items by date")
